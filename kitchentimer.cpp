...
KitchenTimer::KitchenTimer(QObject *parent) : QObject(parent)
  , m_timerId(0)
...
{
    if(m_remainTimeString == remainTimeString) return;
...
{
    if(m_countTime == countTime) return;
...
{
    if(m_runnig == running) return;

    m_runnig = running;
    emit runningChanged(m_runnig);
}
...
{
    setRemainTimeString(QTime::fromMSecsSinceStartOfDay(remainTime).toString("mm:ss"));
}
